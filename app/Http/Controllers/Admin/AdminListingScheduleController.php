...
class AdminListingScheduleController extends Controller
{
    ...
    public function index($id){
        ...
        if($listing){
            if($listing->user_type==1){
                ...
                return view('admin.listing.schedule',compact('listing','schedules','websiteLang','confirmNotify'));
            }else{
                $notification=array(
    ...
    public function create($id){
        ...
        if($listing){
            if($listing->user_type==1){
                return view('admin.listing.create-schedule',compact('listing','days','websiteLang'));
            }else{
                $notification=array(
                    'messege'=>$this->notify->where('id',7)->first()->custom_text,
                    'alert-type'=>'error'
    ...
    public function store(Request $request,$id){
        ...
        if($dayExist ==0){
            ...
            return redirect()->route('admin.listing.schedule',$id)->with($notification);
        }else{
            $notification=array(
    ...
    public function edit($id){
        ...
        if($schedule){
            ...
            if($listing){
                if($listing->user_type==1){
                    $websiteLang=$this->websiteLang;
                    return view('admin.listing.edit-schedule',compact('listing','days','schedule','websiteLang'));
                }else{
                    $notification=array(
                        'messege'=>$this->notify->where('id',7)->first()->custom_text,
                        'alert-type'=>'error'
    ...
    public function update(Request $request,$id){
        ...
        return redirect()->route('admin.listing.schedule',$request->listing_id)->with($notification);
