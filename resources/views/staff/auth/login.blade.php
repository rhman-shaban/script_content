                        ...
                        <div class="col-lg-6">
                            <div class="p-5">
                                <div class="text-center">
                                    <h1 class="h4 text-gray-900 mb-4">{{ $websiteLang->where('id',408)->first()->custom_text }}</h1>
                                </div>
                                <form class="user" id="staffLoginForm">
                                    ...
                                    <div class="form-group">
                                        <input id="exampleInputEmail" type="email" name="email" class="form-control form-control-user"
                                            placeholder="{{ $websiteLang->where('id',38)->first()->custom_text }}">
                                    </div>
                                    <div class="form-group">
                                        <input type="password" name="password" class="form-control form-control-user"
                                            id="exampleInputPassword" placeholder="{{ $websiteLang->where('id',61)->first()->custom_text }}">
                                    </div>
                                    <button type="button" id="staffLoginBtn" class="btn btn-primary btn-user btn-block">
                                        {{ $websiteLang->where('id',58)->first()->custom_text }}
                                    </button>
                                    <hr>
                                </form>
                                <div class="text-center">
                                    <a class="small" href="{{ route('staff.forget.password') }}">{{ $websiteLang->where('id',405)->first()->custom_text }}</a>
                                </div>
                            </div>
                        </div>
